...
function openOptionsForProvider(providerId: string) {
  chrome.storage.local.set({ navigate_to_provider: providerId }).then(() => {
...
const ProviderCard: React.FC<ProviderCardProps> = ({ summary }) => {
  ...
  const lastUpdate = Math.max(
    balanceCache?.lastFetchTimestamp ?? 0,
    statusCache?.lastFetchTimestamp ?? 0,
  );

  return (
    <div
      className="provider-card"
      onClick={() => openOptionsForProvider(provider.id)}
      title={`点击配置 ${config?.displayName || provider.name}`}
    >
      <div className="provider-card-header">
        ...
        <div className="provider-meta">
          <StatusIndicator
            status={statusCache?.result ?? null}
      ...
      <div className="provider-card-body">
        <BalanceDisplay
          balances={balanceCache?.result?.balances ?? []}
          hasApiKey={hasApiKey}
      ...
      <div className="provider-card-footer">
        ...
        {balanceCache?.result?.error && (
          <span className="error-badge" title={balanceCache.result.error}>
            ⚠️
          </span>
        )}
      </div>
